...
// Action runs a cloud sync task now (cloudsync.sync; a job).
...
func (a *Action) Metadata(_ context.Context, req action.MetadataRequest, resp *action.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloudsync_run"
}

func (a *Action) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Runs a cloud sync task now (cloudsync.sync).",
...
func (a *Action) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	...
	if resp.SendProgress != nil {
		resp.SendProgress(action.InvokeProgressEvent{Message: "Running cloud sync…"})
	}
	wait := true
	...
	if _, err := run(ctx, "cloudsync.sync", buildParams(cfg)...); err != nil {
		resp.Diagnostics.AddError("Cloud sync run failed", err.Error())
		return
	}
	if resp.SendProgress != nil {
		msg := "Cloud sync completed."
		if !wait {
