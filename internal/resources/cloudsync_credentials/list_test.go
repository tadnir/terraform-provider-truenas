...
func TestListMetadata(t *testing.T) {
	l := &CredentialsListResource{}
	req := resource.MetadataRequest{ProviderTypeName: "truenas"}
	resp := &resource.MetadataResponse{}
	l.Metadata(context.Background(), req, resp)
	if resp.TypeName != "truenas_cloudsync_credentials" {
		t.Errorf("TypeName = %q, want %q", resp.TypeName, "truenas_cloudsync_credentials")
	}
}

func TestCredentialsIdentitySchema(t *testing.T) {
	r := &CredentialsResource{}
