...
func TestListMetadata(t *testing.T) {
	l := &CronjobListResource{}
	req := resource.MetadataRequest{ProviderTypeName: "truenas"}
	resp := &resource.MetadataResponse{}
	l.Metadata(context.Background(), req, resp)
	if resp.TypeName != "truenas_cronjob" {
		t.Errorf("TypeName = %q, want %q", resp.TypeName, "truenas_cronjob")
	}
}

func TestCronjobIdentitySchema(t *testing.T) {
	r := &CronjobResource{}
