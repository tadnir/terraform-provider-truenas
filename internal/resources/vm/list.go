...
var _ list.ListResource = &VMListResource{}
var _ list.ListResourceWithConfigure = &VMListResource{}

// VMListResource implements the truenas_vm list resource (terraform query).
type VMListResource struct{ client *client.Client }

// NewListResource returns a new VMListResource.
func NewListResource() list.ListResource { return &VMListResource{} }
...
func (l *VMListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	listing.StreamCollection(ctx, l.client, "vm.query", req, stream, l.mapRow)
}

func (l *VMListResource) mapRow(ctx context.Context, req list.ListRequest, raw json.RawMessage) list.ListResult {
	res := req.NewListResult(ctx)
	var api vmAPI
	if err := json.Unmarshal(raw, &api); err != nil {
		res.Diagnostics.AddError("Parse VM row", err.Error())
		return res
	}
